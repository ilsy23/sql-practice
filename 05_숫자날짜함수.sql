...
-- 숫자함수
-- ROUND(반올림)
-- 원하는 반올림 위치를 매개값으로 지정, 음수를 주는 것도 가능(.기준)
SELECT
...
-- 날짜 반올림, 절사
SELECT ROUND(sysdate) FROM dual; -- 하루 절반 넘으면 일 반올림
SELECT ROUND(sysdate, 'year') FROM dual; -- 년 기준으로 반올림
SELECT ROUND(sysdate, 'month') FROM dual; -- 월 기준으로 반올림
SELECT ROUND(sysdate, 'day') FROM dual; -- 일 기준으로 반올림 (일주일 절반 넘으면 반올림) 
