...
-- 서식문자와 함꼐 사용하고 싶은 문자를 ""로 묶어 전달합니다.
SELECT 
...
SELECT TO_CHAR(20000.29, '999999.9') FROM dual;
...
SELECT TO_DATE('2020/12/25', 'YY-MM-DD') FROM dual; -- 형식은 각 숫자가 무엇을 
SELECT TO_DATE('2020*12*25', 'YY-MM-DD') FROM dual; -- 의미하는 지만 나타내면 됨
...
-- NULL 형태를 변환하는 함수 NVL(컬럼, 타겟값) 타겟값 기존 컬럼 타입으로만 가능
SELECT NULL FROM dual; -- (null)
SELECT NVL(NULL, 0) FROM dual; -- (null) -> 0
